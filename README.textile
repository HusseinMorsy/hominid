...
Hominid is intended to be a complete Ruby wrapper for working with the Mailchimp API. As of release 2.0.2, all methods available from the Mailchimp API (Version 1.2) are available. You will need to pass your Mailchimp API key to get started:
...
<pre><code>list)id = h.find_list_id_by_name("Mailing List Name")
list_id_ = h.find_list_id_by_web_id("List Web ID")</code></pre>

This means that you can _(for example)_ get the abuse reports for a particular mailing list:

<pre><code>abuse_reports = h.list_abuse_reports(h.find_list_id_by_name("Mailing List Name"))</code></pre>
